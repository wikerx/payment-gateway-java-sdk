 ...
 */
@Component
public class PayoutWebhookVerifier {
     ...
     */
    public boolean verify(String timestamp, String signature, PayoutWebhookRequest request) {
    ...
    /**
     * 使用 HTTP 原始 query/form 参数校验代付异步通知签名。
     *
     ...
     * 计算代付异步通知签名。
     *
     * 签名原文来自网关当前实现：t + tradeNo + currency + amount + status + code + message。
     * amount 使用 BigDecimal.stripTrailingZeros().toPlainString()，尽量保持金额字符串稳定，避免科学计数法影响商户本地验签。
     *
     * @param timestamp Header `t`
     * @param request 代付回调参数
     * @return SHA-256 hex 小写签名
     */
    public String sign(String timestamp, PayoutWebhookRequest request) {
        String signSource = buildSignSource(timestamp, request);
        return sha256Hex(signSource);
    }

    /**
     * 使用 HTTP 原始 query/form 参数计算代付异步通知签名。
     *
     * @param timestamp Header `t`
     * @param params 原始 query/form 参数首值 Map
     * @return SHA-256 hex 小写签名
     ...
     * @return 签名原文
     */
    public String buildSignSource(String timestamp, PayoutWebhookRequest request) {
    ...
    /**
     * 使用 HTTP 原始 query/form 参数构建代付异步通知签名原文。
     *
     * 当前网关代付回调签名规则为：
     * t + tradeNo + currency + amount + status + code + message。
     * orderNo 会随回调参数返回给商户做幂等和对账，但当前不参与代付回调签名。
     *
     * @param timestamp Header `t`
     * @param params 原始 query/form 参数首值 Map
     * @return 签名原文
