...
/**
 * 代收回调 V2 接收 Controller。
 *
 * 本类负责接收网关推送的加密回调，将验签、解密和 tradeNo 一致性校验委托给 WebhookV2Verifier，
 * 校验通过后再交给商户自定义 PayinWebhookHandler 处理。
 */
@Slf4j
@RestController
@RequestMapping("/api/v2/webhook")
public class PayinWebhookV2Controller {
    ...
    public PayinWebhookV2Controller(WebhookV2Verifier verifier, PayinWebhookHandler handler) {
    ...
    public ResponseEntity<String> receivePayinWebhook(
    ...
    public ResponseEntity<String> handleValidationException(OpenApiValidationException exception) {
        log.warn("代收回调V2-拒绝非法回调: {}", exception.getMessage());
