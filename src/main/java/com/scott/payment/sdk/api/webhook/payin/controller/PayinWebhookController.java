...
public class PayinWebhookController {
    ...
    public ResponseEntity<String> receivePayin(
        ...
        log.info("代收异步通知V1-收到网关回调: {}", JsonSupport.toLogJson(logFields(
        ...
        if (!verifier.verify(timestamp, signature, rawParams)) {
            log.warn("代收异步通知V1-验签失败: {}", JsonSupport.toLogJson(logFields(
        ...
        log.info("代收异步通知V1-处理完成: {}", JsonSupport.toLogJson(logFields(
                "tradeNo", request.getTradeNo(),
                "orderNo", request.getOrderNo(),
                "status", request.getStatus(),
