 ...
 * @email : scott-***@163.com
 * @description : 分页列出客户接口真实网关调用 case，负责使用 merchant-config.properties 创建 SDK 客户端并向测试网关发起
...
public class CustomerListTest {
     ...
     * 真实请求测试环境网关列出客户。
     *
     * 该方法使用 GET + Bearer JWT 调用客户列表接口，不发送请求体；响应 data 由 SDK 解密为客户分页对象。
     */
    @Test
    public void testCustomerList() {
        ...
        OpenApiResult<CustomerPageResponse> result = client.listCustomers();
        ...
        assertThat(result.getData().getList()).isNotNull();
        assertThat(result.getData().getPageNo()).isGreaterThanOrEqualTo(1);
        assertThat(result.getData().getPageSize()).isBetween(1, 100);
    }
}
