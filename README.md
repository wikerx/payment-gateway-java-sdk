...
`payment.gateway.debug-raw-log-enabled=true` 会打印完整请求地址、请求头、请求明文、请求密文、响应密文和响应明文，便于沙盒联调核验。开启后 `Authorization` JWT、加密后的 `data`、卡号、CVC、客户资料、余额和交易状态等数据会进入应用日志，生产环境建议关闭。
...
SDK 只依赖 `slf4j-api`。默认日志仅记录 event、method、path、脱敏商户号、requestId、statusCode、elapsedMillis、success、exceptionType 和报文摘要，不记录完整 JWT、密钥、卡号、CVC、账户号、请求明文、响应明文或完整密文 data。
...
- `payment_gateway_sdk_raw_request`：完整请求地址；
- `payment_gateway_sdk_raw_request_headers`：完整请求头；
- `payment_gateway_sdk_raw_plain_request_body`：加密前请求明文，GET 请求为 `null`；
- `payment_gateway_sdk_raw_request_body`：实际发送请求报文，即加密后的 envelope；
- `payment_gateway_sdk_raw_response`：响应状态、响应头和响应密文报文；
- `payment_gateway_sdk_raw_plain_response_data`：SDK 解密后的响应 data 明文。
